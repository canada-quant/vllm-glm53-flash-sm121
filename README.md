...
Apache-2.0 (this repo's authored content). The image is built on upstream vLLM/FlashInfer Apache-2.0 artifacts. See [`NOTICE.md`](NOTICE.md) — which also documents the friendly-debt line to tonyd2wild's public DGX-Spark bring-up (their repo carries no license; nothing is copied from it, but their published routing insight informed ours).
