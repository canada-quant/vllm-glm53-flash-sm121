...
|---|---|---|
| [vLLM](https://github.com/vllm-project/vllm) @ `385dce36bcee42309924a5ece951a96db3dce7f2` (image `vllm/vllm-openai:glm53-flash-arm64-cu130`, digest `sha256:b0501f99fec5136f248f78d5850977a2ec32d55cd9a665f4a9ffef24cbdf7fe5`) | Apache-2.0 | Base image: GLM-5.3-Flash model support, DFlash2 spec-decode, speculators-format drafter loading, the mask-embedding loader, the top-k −1 init (`sparse_attn_indexer_kpool.py:417`) |
| [FlashInfer](https://github.com/flashinfer-ai/flashinfer) 0.6.18 | Apache-2.0 | Sparse-MLA attention kernels (fa2 path) |
...
The overlay files under `patches/overlays/` are derived from the corresponding
upstream vLLM files at the pinned base and remain Apache-2.0; the modifications
(aux-hidden-state taps; DFLASH2-DRAFTER-GROUP KV fast-path retention) are
canada-quant's, developed for the GLM-5.3-Flash W4A16 dual-DGX-Spark serving stack.
The same hunks were previously published (H200 lane) inside
`canada-quant/dgx-spark-sm121@77ace0b3`.

## Friendly debt (no code copied)

the community bring-up author's public [GLM-5.3-Flash-NVFP4-DFlash2-2x-DGX-Spark](https://GLM-5.3-Flash NVFP4+DFlash2 2x-DGX-Spark community bring-up)
...
## Historical fork file kept for provenance
...
is the fork-era top-k fix file that the authors' fleet bind-mounted into the
now-404 fork image. It is included **for provenance only** — the fix it carried is
subsumed upstream at the pinned base, and this image does NOT use it. It is
derived from vLLM's Apache-2.0 `sparse_attn_indexer_kpool.py` of the fork era.
