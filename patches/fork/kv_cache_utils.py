...
def _get_kv_cache_groups_glm5_next(
    ...
    # extra group below.
    # DFLASH-V1-DRAFTER (port of the H200 v3 hunk — spark-cluster
    # patches/h200-vllm-nightly-dflash2): a full-attention DFlash v1/v2 drafter
    # registers plain FullAttentionSpec layers. GLM-5.3-Flash itself has NONE:
    # its attention is MLAAttentionSpec and its kpool tails are KpoolTailSpec
    # (a SlidingWindowSpec SUBCLASS — excluded here by exact-type `type() is`).
    # Both plain types are therefore drafter layers PRE-conversion.
    # INVARIANT (the discriminator): the only code that can inject TARGET-owned
    # plain FullAttentionSpec is the SW->FA converter unify_hybrid_kv_cache_specs
    # (its only call site is get_kv_cache_groups below, gated on
    # disable_hybrid_kv_cache_manager) — it runs ONLY under
    # --disable-hybrid-kv-cache-manager, which the banked SM121 launchers never
    # set (dead path). A name-membership discriminator is NOT implementable in
    # this path: get_draft_kv_cache_layer_names lives on the drafter model
    # (qwen3_dflash.py:729) and is only called by the speculator
    # (v1/worker/gpu/spec_decode/dflash/speculator.py:197-198), not from here.
    # HARD BAIL (kernel-opt review 2026-09-12T14:44Z): a coexistence-keyed
    # guard is self-defeating — the converter promotes with isinstance, so
    # post-conversion NO SlidingWindowSpec-family specs remain for a conjunct
    # to observe. Condition = the flag ALONE: with it set this fast path is
    # unvalidated full-stop; return None falls through to the general
    # multi-group path and the A3 FAST-PATH TAKEN grep fails the leg closed.
    if vllm_config.scheduler_config.disable_hybrid_kv_cache_manager:
        logger.warning(
            "DFLASH-V1-DRAFTER: --disable-hybrid-kv-cache-manager is set; the "
            "GLM-5-Next fast path is not validated with the SW->FA converter "
            "(target layers may be promoted to plain FullAttentionSpec and "
            "misclassified as drafter). Declining the fast path."
        )
        return None
    draft_specs = {
        k: v
        for k, v in kv_cache_spec.items()
        if type(v) in (SlidingWindowSpec, FullAttentionSpec)
    }
    attn_specs = {
        k: v
        for k, v in kv_cache_spec.items()
        if not isinstance(v, (MambaSpec, KpoolTailSpec))
        and type(v) not in (SlidingWindowSpec, FullAttentionSpec)
    }
    ...
    # A3(a) FAST-PATH TAKEN marker (kernel-opt amendment A3): the only
    # runtime line discriminating a fast-path boot from a silent bail —
    # every earlier return in this function skips it. Emitted only when a
    # drafter group actually formed (a drafter-less serve must not trip the
    # A3 grep). Logging-only; zero grouping semantics.
    if draft_group is not None:
        logger.info(
            "DFLASH2-DRAFTER-GROUP: GLM-5-Next fast path engaged with a "
            "spec-decode drafter group (drafter_layers=%d, mamba_groups=%d, "
            "tail_group=%s)",
            len(draft_specs),
            num_groups,
            tail_group is not None,
        )
...
def _glm5_next_tensor_layout(
    ...
    for g in uniform_groups:
        ...
        if all(type(s) is MLAAttentionSpec for s in group_inner.values()):
            ...
            type(s) in (SlidingWindowSpec, FullAttentionSpec)
            for s in group_inner.values()
        ):
            # DFLASH2-DRAFTER-GROUP / DFLASH-V1-DRAFTER: the spec-decode
            # drafter's SWA or plain-FA group (same partition invariant as
            # _get_kv_cache_groups_glm5_next; validated below once mla_page
            # is known).
            draft_group = g
