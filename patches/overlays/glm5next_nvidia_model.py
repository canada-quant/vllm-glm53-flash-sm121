...
class Glm5NextModel(nn.Module, EagleModelMixin):
    # H200-lane patch (canada-quant, 2026-09-09): auxiliary hidden-state taps for
    # DFlash/EAGLE-3 drafters. Eagle3 index i == the residual stream entering
    # layer i (== output of layer i-1); index start_layer == the embeddings.
    # For deferred-mHC layers the materialized stream is
    # hc_contract(hc_post(x, residual, post, comb), n) -- exactly what the last
    # layer computes before the final norm -- so the taps match the fork's
    # "4 mHC streams -> one hidden_size tensor" capture the drafter was trained on.
    ...
    def forward(
        ...
        **kwargs,
        ...
        if aux_layers and get_pp_group().is_first_rank and self.start_layer in aux_layers:
            aux_hidden_states.append(hidden_states)
        for idx, layer in enumerate(self._active_layers, start=self.start_layer):
            hidden_states, residual, post, comb = layer(
                positions, hidden_states, residual, post, comb
            )
            if aux_layers and (idx + 1) in aux_layers:
                aux_hidden_states.append(
                    self._aux_stream_value(layer, hidden_states, residual, post, comb)
                )
        ...
        if aux_hidden_states:
            if self.is_sequence_parallel:
                aux_hidden_states = [
                    sp_all_gather(t)[:full_num_tokens] for t in aux_hidden_states
                ]
            return hidden_states, aux_hidden_states
        return hidden_states

    @staticmethod
    def _aux_stream_value(
        ...
        # Non-mHC layers return the summed stream; the last mHC layer returns the
        # contracted stream (post is None in both cases). Deferred-mHC layers
        # return the FFN output + the 4-stream residual + the deferred hc_post
        # mix: materialize that post and contract, without touching the state
        # that flows into the next layer's fused post/pre.
        if post is None or comb is None or residual is None:
            return hidden_states
        streams = layer.hc_post(hidden_states, residual, post, comb)
        return hc_contract(streams, layer.n)
