...
def _get_kv_cache_groups_glm5_next(
    ...
    # H200-lane DFLASH2-DRAFTER-GROUP (ported from the sm121 fork): a spec-decode
    # drafter (DFlash/DFlash2) adds plain SlidingWindowSpec layers on top of the
    # GLM-5-Next hybrid. Partition them out (exact type: KpoolTailSpec subclasses
    # SlidingWindowSpec) so they do not disqualify this fast path; appended as one
    # extra group below.
    # DFLASH-V1-DRAFTER: a full-attention DFlash v1 drafter registers plain
    # FullAttentionSpec layers (GLM-5.3-Flash itself has none: its attention is
    # MLAAttentionSpec), so both plain types are drafter layers here.
    draft_specs = {
        name: spec
        for name, spec in kv_cache_spec.items()
        if type(spec) in (SlidingWindowSpec, FullAttentionSpec)
    }
    # GLM5-HIDDEN-GROUP (dspark2 lane): vLLM's `extract_hidden_states`
    # speculative method registers HiddenStateCacheSpec layers (a MLAAttentionSpec
    # subclass) that hold the aux hidden states read out by a KV connector. They
    # must not disqualify this fast path; each becomes its own group LAST, with
    # its page padded to the MLA page (same alignment the generic path applies).
    hidden_specs = {
        name: spec
        for name, spec in kv_cache_spec.items()
    ...
    attn_specs = {
        ...
        and type(spec) not in (SlidingWindowSpec, FullAttentionSpec)
    }
    ...
    # Drafter group (DFLASH2-DRAFTER-GROUP): one extra group for the drafter's
    # SlidingWindowSpec layers, appended LAST so existing group ids stay
    # stable. Never page_size_padded (a padded drafter view is invalid under
    # kernel block splitting). EXACT FIT (drafter page == MLA page after a
    # block-size change): drafter layer i co-owns MLA tensor i at disjoint block
    # ids, like mamba. Otherwise STANDALONE compact per-layer tensors.
    ...
    # GLM5-HIDDEN-GROUP (v4): one group per hidden-state cache layer, SAME block
    # size as the MLA/mamba groups (so every group of this hybrid shares one
    # block size — the V2 hybrid model runner keys alignment off a single block
    # size), and a standalone unpadded tensor region whose page is the natural
    # block_size * bytes-per-token. A hidden page is ~48x an MLA page for
    # 6 taps x 4096 x bf16, so the pool is sized by the hidden layer — which is
    # what an extract_hidden_states engine is for (v1: block 24 padded to the
    # MLA page; that heterogeneous block size is suspected in the 8K IMA).
    hidden_groups: list[KVCacheGroupSpec] = []
    if hidden_specs:
        mla_block_size = mla_specs[mla_names[0]].block_size
        for name, spec in hidden_specs.items():
            per_token = spec.page_size_bytes // spec.block_size
            aligned = replace(spec, block_size=mla_block_size)
            assert aligned.page_size_padded is None
            logger.info(
                "GLM5-HIDDEN-GROUP: hidden-state cache layer %s uses block size %d "
                "(%d B/token), standalone page %d B (%.1fx the MLA page)",
                name,
                mla_block_size,
                per_token,
                aligned.page_size_bytes,
                aligned.page_size_bytes / mla_page,
            )
...
def _glm5_next_tensor_layout(
    ...
    Returns (attn_group, mamba_groups, mla_names, idx_names, mla_page, idx_page,
    tail_names, tail_page, draft_group); draft_group is the DFlash drafter's
    SlidingWindowSpec group or None (H200-lane DFLASH2-DRAFTER-GROUP).
    """
    uniform_groups = [
        group
    ...
    # GLM5-HIDDEN-GROUP: extract_hidden_states cache layers (one group each).
    hidden_groups = [
        group
        for group in kv_cache_groups
    ...
    for group in uniform_groups:
        ...
        if all(type(spec) is MLAAttentionSpec for spec in inner.values()):
            ...
            type(spec) in (SlidingWindowSpec, FullAttentionSpec) for spec in inner.values()
        ):
            draft_group = group
    if attn_group is None or not mamba_groups:
        return None
    if len(uniform_groups) + len(mamba_groups) + len(hidden_groups) != len(
        kv_cache_groups
    ):
        return None
    ...
    if any(group.kv_cache_spec.page_size_bytes != mla_page for group in mamba_groups):
        return None
    if any(group.kv_cache_spec.page_size_padded is not None for group in hidden_groups):
        return None
...
def _glm5_draft_layout(
    draft_group: KVCacheGroupSpec | None, mla_page: int
) -> tuple[list[str], int, bool]:
    """(draft_names, draft_page, shared) for the DFlash drafter group."""
    if draft_group is None:
        return [], 0, False
    draft_page = next(
        iter(
            cast(UniformTypeKVCacheSpecs, draft_group.kv_cache_spec).kv_cache_specs.values()
        )
    ).page_size_bytes
    return list(draft_group.layer_names), draft_page, draft_page == mla_page
...
def _get_kv_cache_bytes_per_block(
    ...
    if (glm5_layout := _glm5_next_tensor_layout(kv_cache_groups)) is not None:
        (
            _, _, mla_names, idx_names, mla_page, idx_page, _, _, draft_group, hidden_groups,
        ) = glm5_layout
        per_block = len(mla_names) * mla_page + len(idx_names) * idx_page
        draft_names, draft_page, draft_shared = _glm5_draft_layout(draft_group, mla_page)
        if draft_names and not draft_shared:
            per_block += len(draft_names) * draft_page
        per_block += sum(g.kv_cache_spec.page_size_bytes for g in hidden_groups)
        return per_block
...
def get_kv_cache_config_from_groups(
    ...
    if (glm5_layout := _glm5_next_tensor_layout(kv_cache_groups)) is not None:
        ...
        # GLM5-HIDDEN-GROUP: one MLA-sized page per hidden-state layer per block.
        bytes_per_block += sum(g.kv_cache_spec.page_size_bytes for g in hidden_groups)
        ...
        draft_specs_inner: dict[str, KVCacheSpec] = (
            cast(UniformTypeKVCacheSpecs, draft_group.kv_cache_spec).kv_cache_specs
            if draft_group is not None
            else {}
        )
        for index, mla_name in enumerate(mla_names):
            ...
            if draft_shared and index < len(draft_names):
                ...
                add_tensor(
                    draft_names[index], draft_specs_inner[draft_names[index]], offset
                )
        ...
        if draft_names and not draft_shared:
            ...
            # regions after the indexer regions.
            draft_base = idx_base + len(idx_names) * idx_page * num_blocks
            for index, draft_name in enumerate(draft_names):
                add_tensor(
                    draft_name,
                    draft_specs_inner[draft_name],
                    draft_base + index * draft_page * num_blocks,
                )
        if hidden_groups:
            # GLM5-HIDDEN-GROUP (standalone): compact per-layer hidden-state
            # regions after the indexer (and any standalone drafter) regions.
            hidden_base = idx_base + len(idx_names) * idx_page * num_blocks
            if draft_names and not draft_shared:
                hidden_base += len(draft_names) * draft_page * num_blocks
            for group in hidden_groups:
                add_tensor(group.layer_names[0], group.kv_cache_spec, hidden_base)
                hidden_base += group.kv_cache_spec.page_size_bytes * num_blocks
...
def _max_memory_usage_bytes_from_groups(
    ...
    if (glm5_layout := _glm5_next_tensor_layout(kv_cache_groups)) is not None:
        ...
        per_block = len(mla_names) * mla_page + len(idx_names) * idx_page
        if draft_group is not None:
            # DFLASH2-DRAFTER-GROUP: charge the drafter's window-bounded block
            # demand; a standalone drafter also adds its pages to every block.
            ...
            draft_names, draft_page, draft_shared = _glm5_draft_layout(
                draft_group, mla_page
            )
            if not draft_shared:
                per_block += len(draft_names) * draft_page
        for group in hidden_groups:
            # GLM5-HIDDEN-GROUP: charge the hidden-state layer's block demand and
            # its per-block page.
            total_blocks += cdiv(
                group.kv_cache_spec.max_memory_usage_bytes(vllm_config),
                group.kv_cache_spec.page_size_bytes,
            )
            per_block += group.kv_cache_spec.page_size_bytes
